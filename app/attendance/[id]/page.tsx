...
import { ChevronLeft, ChevronRight, Flame, CalendarCheck, Clock, RotateCcw, Star, MessageSquare, X } from "lucide-react"
import { fetchApplicationById, submitFeedbackAsClient, submitFeedbackAsAssistant, fetchFeedbackByApplication, FeedbackEntry } from "@/lib/api"
...
export default function AttendancePage() {
  ...
  const [feedbackByDate, setFeedbackByDate] = useState<Map<string, AttendanceStatus>>(new Map())
    ...
    async function loadData() {
      ...
      try {
        ...
        const userIsClient = roleParam 
          ? roleParam === "client" 
          : user.id === (typeof app.clientId === "object" ? app.clientId._id : app.clientId)
        ...
        if (userIsClient) {
          const assistant = typeof app.assistantId === "object" ? app.assistantId : null
          partnerName = assistant?.fullName || `Assistant`
          partnerId = assistant?._id || (typeof app.assistantId === "string" ? app.assistantId : "")
          partnerRole = "assistant"
        } else {
          const client = typeof app.clientId === "object" ? app.clientId : null
          partnerName = client?.fullName || `Client`
          partnerId = client?._id || (typeof app.clientId === "string" ? app.clientId : "")
          partnerRole = "client"
        }
        ...
        const checkinDates = app.checkins?.map((c: { date: string }) => c.date.split("T")[0]) || []
        ...
        const trialInfo: TrialInfo = {
          ...
          partnerRole,
          partnerId,
          startDate: goal?.startDate?.split("T")[0] || app.createdAt.split("T")[0],
  ...
  const today = new Date()
  const initialMonth = trial?.status === "Completed" 
    ? new Date(trial.startDate).getMonth() 
    : today.getMonth()
  const initialYear = trial?.status === "Completed" 
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4">
        ...
        <div className="mt-1 flex items-center gap-2 text-sm">
          ...
          <span className="text-muted-foreground">{trial.partnerRole === "assistant" ? "Assistant" : "Client"}: {trial.partnerName}</span>
        </div>
